# 消融：w/o Sinkhorn OT（把 OT 运输矩阵 T 替换为 普通文本—图像相似度矩阵 Xt @ Xv.T）。
# 在 混合局部改OT核_双类别指标.py（修正版完整模型 + 双类别指标）基础上，唯一改动 = 替换 T 的生成方式：
#   原：T = ot.bregman.sinkhorn_log(a, b, cost, ...)   （Sinkhorn 在代价矩阵 cost 上生成双随机运输矩阵）
#   改：similarity = Xt @ Xv.T → T = softmax(similarity, dim=-1) → T = T / Nt
#        （普通余弦相似度矩阵，softmax 行归一化，再除以 Nt 匹配原 OT 矩阵的总质量尺度 1）
# 目的：验证 Sinkhorn OT 生成的运输矩阵是否优于普通文本—图像相似度矩阵。
#   保留不变：地标投影 OTKernelProjection、TopologyReasoning(关系聚合)、关系代价特征 transport_cost、LoRA、分类器1057维、训练超参。
...
# 仅改动指标计算 + T 的生成方式（消融 w/o Sinkhorn），模型结构、训练、早停、保存最佳模型（仍以fake F1为准）逻辑均与源文件一致。
import os
import json
import math
import time
    ...
    def forward(self, text_input, image_input, return_extra_loss=False):
        ...
        if self.fusion_mode == 'tot':
            ...
            for i in range(t_aug.size(0)):
                ...
                # 消融：w/o Sinkhorn OT —— 不再用 Sinkhorn 生成运输矩阵 T，
                # 改用普通文本—图像相似度矩阵 Xt @ Xv.T（softmax + 质量尺度归一化）。
                similarity = torch.matmul(
                    t_i.float(),
                    v_i.float().transpose(0, 1)
                )

                T = F.softmax(similarity, dim=-1)
