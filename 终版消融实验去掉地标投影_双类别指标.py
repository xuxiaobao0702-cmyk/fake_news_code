# 消融：w/o Landmark Projection（去掉32地标投影 OTKernelProjection）。
# 在 混合局部改OT核.py（修正版完整模型）基础上，唯一改动 = 去掉地标投影：
#   OT代价不再在地标软分配空间(32维)上算，改回在L2归一化原始token特征(512维)上直接算欧氏距离。
#   保留不变：样本级交叉注意力、Sinkhorn(T, reg=0.1)、TopologyReasoning、transport cost、LoRA、分类器1057维。
...
# 仅改动指标计算，消融（去掉地标投影）、模型结构、训练、早停、保存最佳模型（仍以fake F1为准）逻辑均与源文件一致。
import os
import json
import math
import time
...
# ===========================
# [消融] w/o Landmark Projection：
#   整个 OTKernelProjection（32个可学习地标投影）已被移除。
#   原来 OT 代价在"地标软分配关系空间"[B,Nt,32]/[B,Nv,32]上计算；
#   现在直接回退到在 L2 归一化的原始 token 特征(512维)上计算欧氏距离。
#   保留：样本级交叉注意力、Sinkhorn运输T、TopologyReasoning、transport cost、LoRA、分类器1057维。
# ===========================
...
class TopologyReasoning(nn.Module):
    def __init__(self, dim=512, hidden=128):
        ...
        # 融合 文本语义(dim) + OT搬运图像语义(dim) + 拓扑边特征(hidden)，生成具有拓扑感知的新文本节点表示
        # 输入维度: 512 + 512 + 128 = dim*2 + hidden
    ...
    def forward(self, t_tokens, v_tokens, transport_T):
        ...
        v_to_t = torch.mm(T_row, v_tokens)  # V_t = T V：图像语义搬运到文本节点
        ...
        t_edge_agg = (T_row.unsqueeze(-1) * edge_feat).sum(dim=1) #与v_to_t保持同一节点尺度：用行归一化的T_row聚合每个文本Token关联的所有边特征，得到拓扑边特征 E_t
        t_updated = self.node_conv(torch.cat([t_tokens, v_to_t, t_edge_agg], dim=-1))  # T' = f_t([T; V_t; E_t])，更新文本节点表示（图像节点不更新）
...
class StableCLIPModel(nn.Module):
    def __init__(self, num_classes=2, fusion_mode='original', lora_rank=8):
        ...
        # TOT级联模块
        if fusion_mode == 'tot':
            # 消融：w/o Landmark Projection —— 不再实例化 self.ot_kernel
    ...
    def forward(self, text_input, image_input, return_extra_loss=False):
        ...
        if self.fusion_mode == 'tot':
            ...
            for i in range(t_aug.size(0)):
                ...
                # 消融：w/o Landmark Projection —— 不再用32地标投影，
                # 直接在L2归一化的token特征 t_i/v_i (512维) 上算欧氏距离作OT代价
                cost = torch.cdist(t_i.float(), v_i.float(), p=2)
