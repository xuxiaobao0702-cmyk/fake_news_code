#创新3的基础上，将交叉注意力变成全局的，且不是batch的，其余不变
import os
import json
import math
import time
...
class StableCLIPModel(nn.Module):
    ...
    def forward(self, text_input, image_input, return_extra_loss=False):
        ...
        # 步骤1: 全局交叉注意力（每样本独立，文本全局↔图像全局，不跨batch）
        htv, _ = self.attention(ht.unsqueeze(1), hv.unsqueeze(1), hv.unsqueeze(1))
        hvt, _ = self.attention(hv.unsqueeze(1), ht.unsqueeze(1), ht.unsqueeze(1))
        Et = self.norm_text(ht + htv.squeeze(1))
        Ev = self.norm_image(hv + hvt.squeeze(1))
