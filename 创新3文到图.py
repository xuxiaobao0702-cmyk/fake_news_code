...
class TopologyReasoning(nn.Module):
    ...
    ##利用OT构建的跨模态连接关系，以图像节点为主（图到文），学习每个图像Token的连接模式（Topology），最终形成包含全局结构信息的图级特征。
    def forward(self, t_tokens, v_tokens, transport_T):
        ...
        # 图像节点：聚合来自所有文本token的边特征（图为主，文为辅）
        T_t = transport_T.t()  # [Nv, Nt]
        v_edge_agg = (T_t.unsqueeze(-1) * edge_feat.permute(1, 0, 2)).sum(dim=1) #聚合每个图像Token关联的所有边特征，得到拓扑特征
        v_updated = self.node_conv(torch.cat([v_tokens, v_edge_agg], dim=-1))  #将拓扑特征与原始语义特征融合，更新图像节点表示。
        #汇聚所有节点得到图级表示（图为主）
        v_graph = v_updated.mean(dim=0)
        t_graph = t_tokens.mean(dim=0)
