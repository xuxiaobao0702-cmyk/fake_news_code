#创新3: 创新2的基础上加上 LoRA 微调 + 运输成本特征 + 更好训练策略,加上随机种子
import os
import json
import math
import time
...
class OTKernelProjection(nn.Module):
    ...
    def forward(self, text_tokens, image_tokens):
        ...
        for i in range(B):
            ...
            agg_t = mass_t.sum(dim=0, keepdim=True)
            agg_v = mass_v.sum(dim=0, keepdim=True)
            t_out = agg_t.expand(Nt, -1)
            v_out = agg_v.expand(Nv, -1)
...
# ===========================
# TOT创新2: 拓扑感知（加强版：双向传播 + 两跳消息传递 + 注意力池化）
# ===========================
class TopologyReasoning(nn.Module):
    """
    利用OT构建的跨模态二分图进行拓扑推理：
    - 双向传播：文本←→图像，两侧节点都会吸收对方的拓扑信息
    - 两跳消息传递（2-hop GNN）：文本→图像→文本，捕获更深层的图结构
    - 注意力池化：用可学习query对节点加权汇聚，而非简单取均值
    """
    def __init__(self, dim=512, hidden=128, topo_out=128):
        super().__init__()
        # 共享的边编码器（将OT权重映射为高维边特征）
        self.edge_conv = nn.Sequential(
            nn.Linear(1, hidden), nn.ReLU(), nn.Linear(hidden, hidden),
        )
        # 第1跳：文本/图像节点各自吸收对方的拓扑信息
        self.node_conv_t = nn.Sequential(
            nn.Linear(dim + hidden, dim), nn.LayerNorm(dim), nn.ReLU(),
        )
        self.node_conv_v = nn.Sequential(
            nn.Linear(dim + hidden, dim), nn.LayerNorm(dim), nn.ReLU(),
        )
        # 第2跳：基于更新后的特征再次传播，实现文本→图像→文本的多跳推理
        self.node_conv_t2 = nn.Sequential(
            nn.Linear(dim + hidden, dim), nn.LayerNorm(dim), nn.ReLU(),
        )
        self.node_conv_v2 = nn.Sequential(
            nn.Linear(dim + hidden, dim), nn.LayerNorm(dim), nn.ReLU(),
        )
        # 可学习的注意力池化query（让模型自动关注重要的拓扑节点）
        self.t_query = nn.Parameter(torch.randn(1, dim))
        self.v_query = nn.Parameter(torch.randn(1, dim))
        nn.init.trunc_normal_(self.t_query, std=0.02)
        nn.init.trunc_normal_(self.v_query, std=0.02)
        # 图读出层：融合文本图和图像图的全局拓扑特征
        self.graph_readout = nn.Sequential(
            nn.Linear(dim * 2, 128), nn.ReLU(), nn.Linear(128, topo_out),
        )
    ...
    def forward(self, t_tokens, v_tokens, transport_T):
        ...
        # 文本侧：聚合来自所有图像token的边特征
        t_edge_agg = (transport_T.unsqueeze(-1) * edge_feat).sum(dim=1)  # [Nt, hidden]
        t_up1 = self.node_conv_t(torch.cat([t_tokens, t_edge_agg], dim=-1))  # [Nt, D]

        # 图像侧：对称地聚合来自所有文本token的边特征（双向！）
        T_t = transport_T.t()                            # [Nv, Nt]
        v_edge_agg = (T_t.unsqueeze(-1) * edge_feat.permute(1, 0, 2)).sum(dim=1)  # [Nv, hidden]
        v_up1 = self.node_conv_v(torch.cat([v_tokens, v_edge_agg], dim=-1))  # [Nv, D]

        # ========== 第2跳：基于更新后的特征再次传播（多跳推理）==========
        # 用更新后的token特征重新计算软分配矩阵（可微，替代Sinkhorn）
        sim2 = torch.mm(t_up1, v_up1.t()) * 2.0          # [Nt, Nv]
        T2 = F.softmax(sim2, dim=-1)                     # 软分配（每行归一化）
        T2_exp = T2.unsqueeze(-1)
        edge_feat2 = self.edge_conv(T2_exp)              # [Nt, Nv, hidden]

        t_edge_agg2 = (T2.unsqueeze(-1) * edge_feat2).sum(dim=1)  # [Nt, hidden]
        t_up2 = self.node_conv_t2(torch.cat([t_up1, t_edge_agg2], dim=-1))  # [Nt, D]

        v_edge_agg2 = (T2.t().unsqueeze(-1) * edge_feat2.permute(1, 0, 2)).sum(dim=1)  # [Nv, hidden]
        v_up2 = self.node_conv_v2(torch.cat([v_up1, v_edge_agg2], dim=-1))  # [Nv, D]

        # ========== 注意力池化：学习哪些节点对全局拓扑更重要 ==========
        t_attn = F.softmax(torch.mv(t_up2, self.t_query.squeeze(0)), dim=0)  # [Nt]
        t_graph = (t_up2 * t_attn.unsqueeze(-1)).sum(dim=0)                   # [D]

        v_attn = F.softmax(torch.mv(v_up2, self.v_query.squeeze(0)), dim=0)  # [Nv]
        v_graph = (v_up2 * v_attn.unsqueeze(-1)).sum(dim=0)                   # [D]

        # ========== 图读出：拼接文本图和图像图 → 最终拓扑向量 ==========
        graph_feat = torch.cat([t_graph, v_graph], dim=0)   # [2D]
        out = self.graph_readout(graph_feat)                 # [topo_out]
        return out
...
class StableCLIPModel(nn.Module):
    def __init__(self, num_classes=2, fusion_mode='original', lora_rank=8):
        ...
        if fusion_mode == 'tot':
            ...
            # 最终特征: Et(512) + Ev(512) + topo(128) + cost(1) = 1153
            cls_in_dim = 512 + 512 + 128 + 1
