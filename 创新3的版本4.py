#创新3的版本4：在带seed的基础上重新修改了交叉注意力，使其变成局部的
import os
import json
import math
import time
...
class TopologyReasoning(nn.Module):
    def __init__(self, dim=512, hidden=128):
        ...
        self.node_conv = nn.Sequential(    #融合节点自身语义特征与其链接关系特征，生成具有拓扑感知的新节点表示
    ...
    ##利用OT构建的跨模态连接关系，学习每个文本Token的连接模式（Topology），并将这种拓扑信息融入语义表示，最终形成包含全局结构信息的图级特征。
    def forward(self, t_tokens, v_tokens, transport_T):
        Nt, D = t_tokens.shape
        t_agg = torch.mm(transport_T, v_tokens) #每个文本token，用T矩阵加权平均它配对的图像token
        ...
        t_edge_agg = (transport_T.unsqueeze(-1) * edge_feat).sum(dim=1) #聚合每个文本Token关联的所有边特征，得到拓扑特征
        t_updated = self.node_conv(torch.cat([t_tokens, t_edge_agg], dim=-1))  #将拓扑特征与原始语义特征融合，更新节点表示。
        #汇聚所有节点得到图级表示
        t_graph = t_updated.mean(dim=0) 
        v_graph = v_tokens.mean(dim=0)
        graph_feat = torch.cat([t_graph, v_graph], dim=0)
        out = self.graph_readout(graph_feat)
        return out
