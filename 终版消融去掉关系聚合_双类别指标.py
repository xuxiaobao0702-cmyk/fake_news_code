...
# 仅改动指标计算，消融（去掉关系聚合 TopologyReasoning）、模型结构、训练、早停、保存最佳模型（仍以fake F1为准）逻辑均与源文件一致。
import os
import json
import math
import time
...
class StableCLIPModel(nn.Module):
    def __init__(self, num_classes=2, fusion_mode='original', lora_rank=8):
        ...
        if fusion_mode == 'tot':
            ...
            # 消融 w/o 关系聚合：保留地标投影、代价矩阵与 Sinkhorn，删除 TopologyReasoning。
    ...
    def forward(self, text_input, image_input, return_extra_loss=False):
        ...
        if self.fusion_mode == 'tot':
            ...
            for i in range(t_aug.size(0)):
                ...
                # 消融 w/o 关系聚合：不做图到文关系聚合，仅保留运输成本作显式不一致特征
