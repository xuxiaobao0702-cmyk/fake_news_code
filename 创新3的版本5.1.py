#创新3的版本5，在版本3的基础上（修改了拓扑感知部分），将交叉注意力部分进行修改
import os
import json
import math
import time
...
class OTKernelProjection(nn.Module):
    ...
    def forward(self, text_tokens, image_tokens):
        L = F.normalize(self.landmarks, dim=-1)
        B = text_tokens.size(0)
        t_rkhs_list, v_rkhs_list = [], []
        for i in range(B):
            t_i = F.normalize(text_tokens[i], dim=-1)
            v_i = F.normalize(image_tokens[i], dim=-1)
            logit_t = torch.mm(t_i, L.t()) * self.scale
            logit_v = torch.mm(v_i, L.t()) * self.scale
            mass_t = F.softmax(logit_t, dim=-1)
            mass_v = F.softmax(logit_v, dim=-1)
            t_out = mass_t    # 每个token保留自己的地标分配模式，不做全局坍缩
            v_out = mass_v
            t_rkhs_list.append(t_out)
            v_rkhs_list.append(v_out)
        t_rkhs = torch.stack(t_rkhs_list, dim=0)
        v_rkhs = torch.stack(v_rkhs_list, dim=0)
        return t_rkhs, v_rkhs


# ===========================
# TOT创新2: 拓扑感知（改进版：OT语义搬运 + 残差连接）
# ===========================
class TopologyReasoning(nn.Module):
    """
    改进思路：
    1. 保留 edge_conv 对 OT 权重的边特征建模（原有的有效部分）
    2. 新增 OT 加权的跨模态语义搬运：T @ v_tokens / T.t() @ t_tokens
       → 让每个token真正"看到"对方模态它在和谁对话
    3. 残差连接：拓扑增强不覆盖原始CLIP语义，而是增量补充
    4. 双向对称：文本和图像各自做拓扑更新
    """
    def __init__(self, dim=512, hidden=128):
        super().__init__()
        # 边编码器（保留）：将OT权重映射为高维边特征
        self.edge_conv = nn.Sequential(
            nn.Linear(1, hidden), nn.ReLU(), nn.Linear(hidden, hidden),
        )
        # 节点更新器（文本侧）：融合 [自身语义 + 对方模态语义 + 边拓扑特征]
        self.node_conv_t = nn.Sequential(
            nn.Linear(dim * 2 + hidden, dim), nn.LayerNorm(dim), nn.ReLU(),
        )
        # 节点更新器（图像侧）：同上，对称结构
        self.node_conv_v = nn.Sequential(
            nn.Linear(dim * 2 + hidden, dim), nn.LayerNorm(dim), nn.ReLU(),
        )
        # 图读出层：拼接文本图和图像图的全局表示
        ...
        # 残差缩放系数（初始化为0.1，让拓扑信号逐步融入，而非一开始就覆盖原始CLIP语义）
    ...
    def forward(self, t_tokens, v_tokens, transport_T):
        ...
        # ===== 0. 归一化OT矩阵：文本侧行归一化，图像侧列归一化 =====
        # Sinkhorn输出近似双随机，但显式归一化保证各节点接收的信息总量一致
        T_row = transport_T / (transport_T.sum(dim=1, keepdim=True) + 1e-8)  # 行归一化 [Nt, Nv]
        T_col = transport_T / (transport_T.sum(dim=0, keepdim=True) + 1e-8)  # 列归一化 [Nt, Nv]

        # ===== 1. 边特征建模（保留原版）：每条OT边 → 高维向量 =====
        T_exp = transport_T.unsqueeze(-1)                               # [Nt, Nv, 1]
        edge_feat = self.edge_conv(T_exp)                               # [Nt, Nv, hidden]

        # ===== 2. 边特征聚合到各节点（使用归一化后的权重）=====
        # 文本节点：用行归一化T聚合图像侧的边特征
        t_edge_agg = (T_row.unsqueeze(-1) * edge_feat).sum(dim=1)      # [Nt, hidden]

        # 图像节点：用列归一化T聚合文本侧的边特征
        v_edge_agg = (T_col.t().unsqueeze(-1) * edge_feat.permute(1, 0, 2)).sum(dim=1)  # [Nv, hidden]

        # ===== 3. ★OT加权跨模态语义搬运（使用归一化后的权重）=====
        # 核心想法：OT矩阵的本职工作是"搬运"——把对方模态语义按配比搬过来
        t_from_v = torch.mm(T_row, v_tokens)                            # [Nt, D]  文本看到图像语义
        v_from_t = torch.mm(T_col.t(), t_tokens)                        # [Nv, D]  图像看到文本语义

        # ===== 4. 融合 + 残差更新 =====
        # 拼接 [自身语义, 对方模态语义, 边拓扑特征] → 更新
        t_updated = self.node_conv_t(
            torch.cat([t_tokens, t_from_v, t_edge_agg], dim=-1)
        )                                                                # [Nt, D]
        t_final = t_tokens + self.alpha_t * t_updated                    # 可学习缩放的残差连接
        t_final = F.normalize(t_final, dim=-1)

        v_updated = self.node_conv_v(
            torch.cat([v_tokens, v_from_t, v_edge_agg], dim=-1)
        )                                                                # [Nv, D]
        v_final = v_tokens + self.alpha_v * v_updated                    # 可学习缩放的残差连接
        v_final = F.normalize(v_final, dim=-1)

        # ===== 5. 池化 + 图读出 =====
        t_graph = t_final.mean(dim=0)                                    # [D]
        v_graph = v_final.mean(dim=0)                                    # [D]
        graph_feat = torch.cat([t_graph, v_graph], dim=0)                # [2D]
        out = self.graph_readout(graph_feat)                             # [32]
        return out
...
class StableCLIPModel(nn.Module):
    ...
    def forward(self, text_input, image_input, return_extra_loss=False):
        ...
        text_tokens = text_outputs.last_hidden_state[:, 1:, :]          # [B, Nt, 512]
        image_tokens = image_outputs.last_hidden_state[:, 1:, :]        # [B, Nv, 768]
        image_tokens = self.image_proj(image_tokens)                    # [B, Nv, 512]
        text_tokens = F.normalize(text_tokens, dim=-1)
        image_tokens = F.normalize(image_tokens, dim=-1)

        # ============ token级双向交叉注意力 ============
        # 文本token读取图像patch信息
        t_cross, _ = self.attention(
            query=text_tokens,
            key=image_tokens,
            value=image_tokens
        )
        # 图像patch读取文本token信息（mask掉padding token）
        text_padding_mask = (text_input["attention_mask"][:, 1:] == 0)  # [B, Nt]
        v_cross, _ = self.attention(
            query=image_tokens,
            key=text_tokens,
            value=text_tokens,
        ...
        # 残差连接
        t_aug = self.norm_text(text_tokens + t_cross)
        v_aug = self.norm_image(image_tokens + v_cross)
        t_aug = F.normalize(t_aug, dim=-1)
        v_aug = F.normalize(v_aug, dim=-1)

        # ============ 生成全局Et/Ev（token池化 + CLIP全局语义）============
        # 文本有效token池化（排除padding）
        text_valid_mask = text_input["attention_mask"][:, 1:].unsqueeze(-1).float()  # [B, Nt, 1]
        t_global = (t_aug * text_valid_mask).sum(dim=1) / (text_valid_mask.sum(dim=1) + 1e-8)  # [B, 512]
        # 图像patch池化
        v_global = v_aug.mean(dim=1)                                                # [B, 512]
        # 融合CLIP原始全局语义，保留全局层面的对齐信号
        Et = F.normalize(ht + t_global, dim=-1)
        Ev = F.normalize(hv + v_global, dim=-1)

        # ============ TOT级联路径 ============
        if self.fusion_mode == 'tot':
            t_rkhs, v_rkhs = self.ot_kernel(t_aug, v_aug) #OT核投影

            topo_feats, cost_feats = [], []
            for i in range(t_rkhs.size(0)):
                cost = torch.cdist(t_rkhs[i].float(), v_rkhs[i].float(), p=2)
                ...
                topo = self.topology(t_aug[i], v_aug[i], T)
