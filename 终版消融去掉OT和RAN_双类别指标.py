# 消融：同时去掉 OT 关系建模 + RAN 关系聚合（w/o OT & RAN）。
# 在 混合局部改OT核_双类别指标.py（修正版完整模型 + 双类别指标）基础上，去掉整个 "OT + 关系聚合" 分支：
#   去掉：self.ot_kernel（地标投影）、self.topology（TopologyReasoning 关系聚合）、Sinkhorn、T矩阵、
#         topo_feats、cost_feats、transport_cost。
#   保留：样本级交叉注意力（Et/Ev 粗对齐特征）、LoRA、对比损失、分类器（输入维度改为 512+512=1024）、训练超参。
#   目的：验证整个 "OT关系建模 + 关系聚合" 分支是否有效。
...
# 仅改动指标计算 + 去掉 OT/RAN 分支，样本级交叉注意力、训练、早停、保存最佳模型（仍以fake F1为准）逻辑均与源文件一致。
import os
import json
import math
import time
...
# ===========================
# [消融] w/o OT & RAN：
#   整个 "OT 关系建模（OTKernelProjection 地标投影 + Sinkhorn 运输矩阵 T + transport_cost）
#   + RAN 关系聚合（TopologyReasoning）" 分支已被移除。
#   得到 Et/Ev（样本级交叉注意力粗对齐特征）后直接拼接分类，分类器输入维度 512+512=1024。
# ===========================
...
class StableCLIPModel(nn.Module):
    def __init__(self, num_classes=2, fusion_mode='original', lora_rank=8):
        ...
        # TOT级联模块
        if fusion_mode == 'tot':
            # 消融：同时去掉 OT 关系建模（ot_kernel/Sinkhorn/T/transport_cost）与 RAN 关系聚合（topology）。
            # 最终特征仅剩 Et(512) + Ev(512) = 1024，直接分类。
            cls_in_dim = 512 + 512
    ...
    def forward(self, text_input, image_input, return_extra_loss=False):
        ...
        # ============ TOT级联路径（消融：去掉 OT 关系建模 + RAN 关系聚合） ============
        if self.fusion_mode == 'tot':
            # 得到 Et/Ev 后直接拼接分类，不再做 OT 运输矩阵、图到文关系聚合、运输成本
            final_feat = torch.cat([Et, Ev], dim=-1)

            assert final_feat.size(-1) == 1024

            logits = self.classifier(final_feat)

            return logits, Et, Ev, None
